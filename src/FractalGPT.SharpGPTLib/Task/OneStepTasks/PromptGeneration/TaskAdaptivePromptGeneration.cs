...
namespace FractalGPT.SharpGPTLib.Task.PromptGeneration
{
    ...
    /// </summary>
    [Serializable]
    public class TaskAdaptivePromptGeneration
    {
        private readonly IText2TextAPI _text2Text;
        private readonly string _lang;

        /// <summary>
        /// Initializes a new instance of the <see cref="TaskAdaptivePromptGeneration"/> class with the given text API.
        /// </summary>
        ...
        public TaskAdaptivePromptGeneration(IText2TextAPI text2Text, string lang)
        {
            _lang = lang;
            _text2Text = text2Text ?? throw new ArgumentNullException(nameof(text2Text));
            // Initialize with a default system prompt.
            _text2Text.SetPrompt(PromptManager.SystemPrompts["task_prompt_generation_en"]);
        }
        ...
        {
            var prompt_dict = PromptManager.TasksForChatModelPrompts;
            string promts = "";
            string taskD = "Task name";
            string promptText = "Prompt";

            foreach (var prompt in prompt_dict)
                promts += $"{taskD}: {prompt.Key}\t{promptText}: {prompt.Value}\n";

            return TaskPrompts.InputPrompt(promts, "prompt_generation", _lang) + task;
        }
    }
}